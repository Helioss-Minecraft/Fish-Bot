...
initial_extensions = ["admin", "timeclass", "errors",
                      "general", "sconfig", "configManager", "voices", "logs", "perms", "thanks", "ptero", "nickaspiring", "extloader", "linkusernames"]
...
def main():
    with open('config.json') as f:
        conf = json.load(f)
    client = FishBot(case_insensitive=True, status=discord.Status(
    ...
    async def on_ready():
        ...
        print('------')
        client.get_cog("NickAspiring").nick.start()
